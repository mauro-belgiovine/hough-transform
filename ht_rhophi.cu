...
__global__ void voteHoughSpace(float *dev_x_values, float *dev_y_values, float *dev_z_values, int *dev_accMat, float dtheta, float drho, float dphi){
  ...
  float theta=acos(z_val/(R2+z_val*z_val));
...
__global__ void findRelativeMax(int *dev_accMat, struct track_param *dev_output,/* struct Lock mutex,*/ unsigned int *NMrel){
...
//    read_inputFile("../datafiles/hits-1.txt");
    ...
    //float out_tracks[ Nsec ][ Ntheta ][ Nphi ] [ Nrho ];
    ...
    //checkCudaErrors(cudaMalloc((void **) &my_lock, (sizeof(struct Lock))));
