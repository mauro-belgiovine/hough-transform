...
__global__ void findRelativeMax(int *dev_accMat, struct track_param *dev_output, unsigned int *NMrel){
	...
	unsigned int ith = blockIdx.x / (Nrho/blockDim.y); //instead of blockIdx.y / (...etc...)
	unsigned int iphi = threadIdx.x;
	unsigned int irho = blockIdx.x % (Nrho/blockDim.y);
  ...
  //we must check from ith >= 1, iphi >= 1, irho >= 1
    ...
    //each thread is assigned to one point of the accum. matrix:
		int acc= dev_accMat[get3DIndex(ith, iphi, irho)];

		if (acc >= ac_soglia){

			if(acc > dev_accMat[get3DIndex(ith,iphi, irho-1)] && acc >= dev_accMat[get3DIndex(ith, iphi, irho+1)]){

				if(acc > dev_accMat[get3DIndex( ith, iphi-1, irho)] && acc >= dev_accMat[get3DIndex(ith, iphi+1, irho)]){
...
class ht_gpu_data{
	...
	public:
		...
		void accMat_init(){
			size_t memsize = Ntheta*Nphi*Nrho;
			//first, set proper GPU device id
			checkCudaErrors(cudaSetDevice(id));
		...
		void find_RelativeMax(){
		    ...
		    unsigned int dim_x_grid = Ntheta * (Nrho/dim_y_block);

		    dim3 grid(dim_x_grid);
